...
maxlik <- function(datmat, exo=1, instrument=1){
    ...
    maxboots <- function(bootsize=599, eevs= 1, exo=1, instrument=1){
      bootse <- c()
      for (p in 1:bootsize){
        ...
        bootlik=function(theta){
          ...
          rho = (exp(2*iht)-1)/(exp(2*iht)+1)
          #rho = tanh(iht)
          m = (alpha1+bootx[1]*beta+bootxo%*%gamma+(rho*(bootx[1]-alpha2-bootxo%*%pi1-bootz%*%pi2)/exp(logsig))/(sqrt(1-(rho^2))))
          logl = y*log(pnorm(m,mean=0,sd=1))+(1-y)*log(1-pnorm(m,mean=0,sd=1))+log(dnorm(((bootx[1]-alpha2-bootxo%*%pi1-bootz%*%pi2)/exp(logsig)),mean=0,sd=1))-logsig
          return(-sum(logl))
        }
        tryCatch({
    ...
    for (j in 1:ncol(datmat)){  
      for (i in 1:nrow(datmat)){
        ...
        cfse = maxboots()
        c8[i,j] <- cover(estimate = r8[i,j], se=cfse)      
      }  
